...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.down_conv_1 = double_conv_3d(3, 64)
...
maps_list = [None]*50
for l in range(1,51):
    file = open(r"theirLandmarks/LMs_"+ str(l) + ".txt")
    file.readline()
    file.readline()
    file.readline()
    file.readline()
    X1Y1Z1 = file.readline()
    X2Y2Z2 = file.readline()
    X1Y1Z1 = X1Y1Z1.split()
    X2Y2Z2 = X2Y2Z2.split()
    
    X1 = int(float(X1Y1Z1[0]))
    Y1 = int(float(X1Y1Z1[2]))
    Z1 = int(float(X1Y1Z1[1]))
    X2 = int(float(X2Y2Z2[0]))
    Y2 = int(float(X2Y2Z2[2]))
    Z2 = int(float(X2Y2Z2[1]))
    file.close()
    
    distance_map = np.zeros((256,256,256))
    distance_map1 = np.zeros((256,256,256))
    distance_map2 = np.zeros((256,256,256))
