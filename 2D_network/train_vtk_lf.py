...
def load_data(vtk_dir="vtk_files",add_landmarks=False,organ='liver'):
    ...
    for i in range(len(body_array)):
        my_list = torch.sum(body_array[i][2,:,:,:],dim=(0,2))>0
        indices = [j for j, x in enumerate(my_list) if x == True]
        if len(indices)!=0:
            body_array_new.append(body_array[i][:,:,min(indices):max(indices),:])
        else:
            # if no slice contain a pixel of organ then add 5 slices so that we keep the same number of bodies
...
def train_net(model,body_array_new,maps_list,epochs,device,lr,batch_size=10):
    ...
    for epoch in range(epochs):
        ...
        maps_lf = torch.zeros(1,1,x_range,z_range).to(device='cuda')
        for sample in val_list:
            x_eval[0,0:2,:,:] = body_array_new[sample[0]][0:2,:,sample[1],:]
            maps_lf[0,0,:,:] = torch.from_numpy(np.asarray(maps_list[sample[0]]))
            y_eval = body_array_new[sample[0]][2:,:,sample[1],:].to(device='cuda')
            with torch.no_grad():
                y_pred = model(x_eval,maps_lf)
...
def eval_net(model,body_array_new, maps_list, device):
    ...
    for body,slc in smart_list:
        ...
        maps_lf[0,0,:,:] = torch.from_numpy(np.asarray(maps_list[body]))
...
if __name__=='__main__':
    ...
    train_data, test_data, maps = load_data(organ=args.organ,add_landmarks=True)        
