...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.up_conv_4 = double_conv(129, 64)
    ...
    def forward(self, image):
        x1 = self.down_conv_1(image[:,0:2,:,:]).squeeze(3)
        ...
        x = self.up_trans_4(x)
        #print(image[:,2,:,2,:].unsqueeze(1).size())
        #print(x.size())
        #print(x1.size())
        x = self.up_conv_4(torch.cat( (image[:,2,:,:].unsqueeze(1),x, x1),1 ))
        x = self.out(x)
        x = self.soft(x)
        return x
